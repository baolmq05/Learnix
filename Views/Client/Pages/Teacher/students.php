  ...
  <div class="flex items-center justify-between mb-6">
    ...
    <div class="flex items-center gap-2">
      <label class="relative block">
        ...
        <input
          class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
          placeholder="Tìm kiếm tên hoặc email..." type="text" name="q" />
        <svg class="absolute left-2 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" xmlns="http://www.w3.org/2000/svg"
          ...
          <tr class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-sm text-gray-600">
              <div class="w-40 bg-gray-100 rounded-full h-3 overflow-hidden">
                <div
                  class="h-2 bg-linear-to-r from-purple-600 via-blue-400 to-purple-600 rounded-full transition-all duration-700 ease-out"
                  style="width: <?= max(0, min(100, (int) $s['progress'])) ?>%;"></div>
              </div>
              <div class="text-xs text-gray-500 mt-1"><?= (int) $s['progress'] ?>%</div>
            </td>
            <td class="px-4 py-3 text-sm">
              <?php if ($s['status'] === 'completed'): ?>
                <span class="inline-flex items-center px-2 py-1 rounded text-xs bg-green-100 text-green-800">Hoàn
                  thành</span>
              <?php else: ?>
                <span class="inline-flex items-center px-2 py-1 rounded text-xs bg-yellow-100 text-yellow-800">Đang
                  học</span>
              <?php endif; ?>
            </td>
          </tr>
