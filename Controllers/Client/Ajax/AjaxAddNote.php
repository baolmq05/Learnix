...
$videoTime = $_POST['timeInVideo'] ?? null;
$noteModel = new Note();
if (!$userId) {
    echo json_encode([
        "status" => "error",
        "message" => "Vui lòng đăng nhập để thêm ghi chú!"
    ]);
    exit;
}
if (empty($userId) || !isset($lessonId) || !isset($content) || !isset($videoTime)) {
    echo json_encode([
        "status" => "error",
...
$addNote = $noteModel->addNote($userId, $lessonId, $content, $videoTime);
?>
<?php
if ($addNote):
